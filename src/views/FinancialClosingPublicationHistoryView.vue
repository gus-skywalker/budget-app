<template>
  <div class="cb-page"><div class="cb-container history">
    ...
    <section v-else-if="review && progress" class="cb-card">
      <p class="eyebrow">Gestão da base editável</p><h1>Lote materializado</h1>
      ...
      <p v-if="progress.contributingPublication">Este é o detalhe de um único lote. Enquanto a publicação financeira não foi confirmada, ele ainda pode ser retirado da competência e seu histórico será preservado.</p>
      ...
      <dl><div><dt>Fontes neste lote</dt><dd>{{ review.selectedSourceCount }}</dd></div><div><dt>Fontes ativas na competência</dt><dd>{{ publishedCompetenceSources.length }}</dd></div><div><dt>Itens deste lote</dt><dd>{{ progress.publishableItemCount ?? review.sources.reduce((total, source) => total + source.candidateCount, 0) }}</dd></div></dl>
      ...
      <div class="actions"><v-btn color="primary" @click="back">Voltar às fontes da competência</v-btn><v-btn variant="tonal" @click="openRules">Escolher fonte para regras</v-btn><v-btn v-if="canCancelMaterialization" color="error" variant="tonal" :loading="cancelling" @click="cancelMaterialization">Retirar este lote da competência</v-btn></div>
    </section>
  </div></div>
</template>
...
function openRules(){router.push({name:'closing-rules',params:{closingId:String(route.params.closingId)}})}
async function cancelMaterialization(){if(!closing.value||!review.value||!window.confirm('Retirar este lote da competência? O histórico será preservado, mas seus itens deixarão de compor o próximo cálculo.'))return;cancelling.value=true;try{await FinancialClosingService.cancelWorkbookMaterialization(closing.value.id,review.value.id);back()}catch{error.value='Não foi possível retirar este lote. Atualize a competência e tente novamente.'}finally{cancelling.value=false}}
