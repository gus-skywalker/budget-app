  ...
  <div class="cb-page closing-page">
    <div class="cb-container">
      ...
      <template v-else-if="summary && matrix">
        ...
        <section v-if="userStore.canWrite" class="cb-card obligations-card manual-payment"><div class="section-heading"><div><p class="eyebrow">Registro interno</p><h2>Pagamento manual</h2></div></div><p class="matrix-hint">Este registro não realiza uma transferência bancária.</p><div class="payment-form"><v-text-field v-model.number="paymentAmount" label="Valor" type="number" min="0.01" /><v-text-field v-model="paymentReference" label="Referência interna" /><v-text-field v-model="paymentExecutedAt" label="Data e hora da ocorrência" type="datetime-local" /><v-btn color="primary" :loading="savingPayment" @click="recordPayment">Registrar como rascunho</v-btn></div><div v-if="recordedPayment"><p class="matrix-hint">Rascunho {{ recordedPayment.id }} · {{ money(recordedPayment.amount) }}</p><v-textarea v-if="userStore.isAdmin" v-model="confirmationJustification" label="Justificativa da confirmação" /><v-text-field v-if="userStore.isAdmin" v-model="confirmationEvidence" label="Evidência (não será exibida depois)" /><div class="payment-form"><v-btn v-if="userStore.isAdmin" color="success" :disabled="!confirmationJustification || !confirmationEvidence" @click="confirmPayment">Confirmar fato interno</v-btn><v-btn color="error" variant="tonal" @click="voidPayment">Anular rascunho</v-btn></div></div></section>
        <section v-if="suggestionPaymentId || bankSuggestions" class="cb-card obligations-card bank-suggestions" aria-labelledby="bank-suggestions-title"><div class="section-heading"><div><p class="eyebrow">Conciliação bancária assistida</p><h2 id="bank-suggestions-title">Sugestões de extrato</h2></div><v-btn v-if="suggestionPaymentId" variant="tonal" :loading="loadingSuggestions" @click="loadBankSuggestions">Atualizar</v-btn></div><p class="matrix-hint">Sugestão somente leitura: não confirma extrato, não altera saldos e não executa operação bancária.</p><div v-if="loadingSuggestions" class="cb-empty-state"><v-progress-circular indeterminate size="24" /><p>Buscando candidatos seguros…</p></div><p v-else-if="suggestionError" class="matrix-hint" role="alert">{{ suggestionError }}</p><p v-else-if="bankSuggestions?.classification === 'NO_MATCH'" class="matrix-hint">Nenhum lançamento bancário elegível foi encontrado para esta execução.</p><div v-else-if="bankSuggestions" class="matrix-scroll"><table class="closing-matrix"><thead><tr><th>Classificação</th><th>Valor</th><th>Data</th><th>Direção</th><th>Explicação</th></tr></thead><tbody><tr v-for="candidate in bankSuggestions.candidates" :key="candidate.candidateId"><th><v-chip size="x-small" :color="candidate.classification === 'EXACT' ? 'success' : candidate.classification === 'AMBIGUOUS' ? 'warning' : 'info'" variant="tonal">{{ suggestionLabel(candidate.classification) }}</v-chip></th><td>{{ money(candidate.amount) }}</td><td>{{ formatDate(candidate.transactionDate) }}</td><td>{{ candidate.direction === 'OUTFLOW' ? 'Saída' : 'Entrada' }}</td><td><span class="suggestion-reasons">{{ candidate.reasons.map(reasonLabel).join(' · ') }}</span></td></tr></tbody></table></div></section>
        <section v-if="userStore.canWrite && obligations.length" class="cb-card obligations-card"><div class="section-heading"><div><p class="eyebrow">Fato compensatório</p><h2>Correção financeira</h2></div></div><p class="matrix-hint">A correção cria um fato novo; não altera obrigações ou pagamentos históricos.</p><div class="payment-form"><v-select v-model="correctionType" :items="correctionTypes" label="Efeito" /><v-select v-model="correctionObligationId" :items="obligations.map(item => ({ title: `${item.domain} · ${money(item.openAmount)}`, value: item.id }))" label="Obrigação de origem" /><v-text-field v-model.number="correctionAmount" label="Valor" type="number" min="0.01" /><v-btn color="primary" :disabled="!correctionObligationId || !correctionAmount" @click="createCorrection">Criar para revisão</v-btn></div><div v-if="correction"><p class="matrix-hint">{{ correction.type }} · {{ correction.status }}</p><div class="payment-form"><v-btn v-if="correction.status === 'DRAFT'" @click="submitCorrection">Enviar para revisão</v-btn><v-btn v-if="correction.status === 'REVIEW' && userStore.isAdmin" color="success" @click="approveCorrection">Aprovar fato compensatório</v-btn></div></div></section>
      </template>
    </div>
...
import FinancialClosingService, { type BankReconciliationSuggestionPage, type CalculationMemory, type ClosingMatrix, type ClosingSource, type ClosingSummary, type DrillDown, type FinancialClosing, type MatrixRow, type OperationalObligation, type PaymentExecution, type FinancialCorrection } from '@/services/FinancialClosingService'
...
const suggestionPaymentId = ref(''); const bankSuggestions = ref<BankReconciliationSuggestionPage | null>(null); const loadingSuggestions = ref(false); const suggestionError = ref('')
const correctionType = ref('RELEASE_OPEN_PRINCIPAL'); const correctionObligationId = ref(''); const correctionAmount = ref<number | null>(null); const correction = ref<FinancialCorrection | null>(null); const correctionTypes = [{ title: 'Crédito contra principal aberto', value: 'RELEASE_OPEN_PRINCIPAL' }, { title: 'Obrigação complementar', value: 'ISSUE_SUPPLEMENT' }, { title: 'Obrigação de recuperação', value: 'ISSUE_RECOVERY' }]
...
async function loadBankSuggestions() { if (!suggestionPaymentId.value) return; loadingSuggestions.value = true; suggestionError.value = ''; try { bankSuggestions.value = (await FinancialClosingService.bankReconciliationSuggestions(suggestionPaymentId.value)).data } catch { bankSuggestions.value = null; suggestionError.value = 'Não foi possível consultar as sugestões bancárias.' } finally { loadingSuggestions.value = false } }
async function createCorrection() { if (!correctionObligationId.value || !correctionAmount.value) return; try { correction.value = (await FinancialClosingService.createCorrection({ type: correctionType.value, originalObligationId: correctionObligationId.value, amount: correctionAmount.value })).data } catch { errorMessage.value = 'Não foi possível criar a correção.' } }
...
async function approveCorrection() { if (!correction.value) return; try { correction.value = (await FinancialClosingService.approveCorrection(correction.value.id, correction.value.revision)).data; await loadObligations() } catch { errorMessage.value = 'Não foi possível aprovar a correção.' } }
