...
import FinancialClosingService, { type CalculationMemory, type ClosingMatrix, type ClosingSource, type ClosingSummary, type DrillDown, type FinancialClosing, type MatrixRow, type OperationalObligation, type PaymentExecution, type FinancialCorrection } from '@/services/FinancialClosingService'
...
const paymentAmount = ref<number | null>(null); const paymentReference = ref(''); const savingPayment = ref(false); const recordedPayment = ref<PaymentExecution | null>(null); const confirmationJustification = ref(''); const confirmationEvidence = ref('')
...
async function recordPayment() { if (!paymentAmount.value || paymentAmount.value <= 0) return; savingPayment.value = true; try { recordedPayment.value = (await FinancialClosingService.recordPayment({ direction: 'OUTBOUND', currency: selectedClosing.value?.currency || 'BRL', amount: paymentAmount.value, externalReference: paymentReference.value || undefined, idempotencyKey: crypto.randomUUID() })).data } catch { errorMessage.value = 'Não foi possível registrar o rascunho.' } finally { savingPayment.value = false } }
async function confirmPayment() { if (!recordedPayment.value) return; try { await FinancialClosingService.confirmPayment(recordedPayment.value.id, { justification: confirmationJustification.value, evidenceReference: confirmationEvidence.value, idempotencyKey: crypto.randomUUID() }); recordedPayment.value = null; confirmationEvidence.value = ''; confirmationJustification.value = ''; await loadObligations() } catch { errorMessage.value = 'Não foi possível confirmar o fato interno.' } }
