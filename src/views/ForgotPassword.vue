        ...
        <div class="auth-shell single-column">
            <section class="auth-card">
                ...
                <div class="copy-block">
                    <span class="section-tag">{{ $t('authentication.forgot_password.title') }}</span>
                    <h1>{{ $t('authentication.forgot_password.title') }}</h1>
                    <p>{{ $t('authentication.forgot_password.description') }}</p>
                </div>

                <form class="auth-form" @submit.prevent="handleForgotPassword">
                    <div class="form-group">
                        <label for="email">{{ $t('authentication.forgot_password.email_label') }}</label>
                        <input
                            id="email"
                            v-model="email"
                            type="email"
                            :placeholder="$t('authentication.forgot_password.email_placeholder')"
                            required
                        />
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary" :disabled="isLoading">
                        ...
                        <span v-else>{{ $t('authentication.forgot_password.submit_button') }}</span>
                    </button>
                </form>
...
const handleForgotPassword = async () => {
    ...
    if (!emailRegex.test(email.value)) {
        message.value = t('authentication.forgot_password.invalid_email')
    ...
    try {
        ...
        message.value = t('authentication.forgot_password.success_message')
        messageType.value = 'success'
    } catch (err) {
        console.error(err)
        message.value = t('authentication.forgot_password.generic_error')
        messageType.value = 'error'
    } finally {
