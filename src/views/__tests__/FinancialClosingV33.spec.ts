...
describe('Financial closing V33 rules and result journey',()=>{
  ...
  it('calculates only through the canonical endpoint and renders PB, deductions, PL and payout',async()=>{
    ...
    serviceMock.calculate.mockResolvedValue({data:calculated});const wrapper=mount(FinancialClosingResultView,{global:{plugins:[vuetify],stubs}});await flush();await flush();expect(wrapper.text()).toContain('Prévia pronta para calcular');await (wrapper.vm as any).calculate();await flush();expect(serviceMock.calculate).toHaveBeenCalledWith(closing);expect(wrapper.text()).toContain('Cálculo atual');expect(wrapper.text()).toContain('Valor a Receber');expect(wrapper.text()).toContain('R$ 76,50');expect(wrapper.text()).toContain('Reserva TM · regra explícita');expect(wrapper.text()).toContain('Valor bruto − reversões − deduções da fonte')
  })
