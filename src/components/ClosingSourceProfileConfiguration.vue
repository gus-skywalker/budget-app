<template>
  <section class="source-configuration" aria-labelledby="source-configuration-title">
    ...
    <section v-if="headerCandidates.length > 1" class="header-row-selection" aria-labelledby="header-row-title">
      <h3 id="header-row-title">Onde começam as colunas?</h3>
      <p>Escolha a linha que nomeia as colunas desta aba. A prévia abaixo muda para essa estrutura; nenhum dado é importado nesta etapa.</p>
      <v-select v-model="headerRow" :items="headerRowOptions" label="Linha dos cabeçalhos" />
    </section>
    <alert-strip v-else-if="!headerCandidates.length" variant="info" description="Não foi possível reconhecer uma linha de cabeçalhos nesta aba. Escolha uma aba tabular ou ajuste a planilha antes de configurar a fonte." />
...
const headerCandidates=computed(()=>props.sheet.headerCandidates||[])
const defaultHeaderRow=computed(()=>headerCandidates.value.find(candidate=>JSON.stringify(candidate.headers)===JSON.stringify(props.sheet.headers||[]))?.rowNumber||headerCandidates.value[0]?.rowNumber)
const headerRow=ref<number|undefined>(defaultHeaderRow.value)
const selectedHeaderCandidate=computed(()=>headerCandidates.value.find(candidate=>candidate.rowNumber===headerRow.value))
const headers=computed(()=>selectedHeaderCandidate.value?.headers||props.sheet.headers||[])
const headerRowOptions=computed(()=>headerCandidates.value.map(candidate=>({title:`Linha ${candidate.rowNumber} · ${candidate.headers.slice(0,3).join(' · ')}`,value:candidate.rowNumber})))
const optionalHeaders=computed(()=>[{title:'Não existe nesta fonte',value:''},...headers.value.map(header=>({title:header,value:header}))])
