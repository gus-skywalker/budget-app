...
const sheet:any={sheetName:'Source one',classification:'SUPPORT_REVIEW',suggestedSourceKey:'SOURCE_ONE',nonEmptyDataRows:2,detail:'Needs configuration',selectionStatus:'REVIEW_REQUIRED',headers:['reference','amount','date','responsible'],headerCandidates:[{rowNumber:2,headers:['reference','amount','date','responsible']},{rowNumber:5,headers:['other','value','when','owner']}],mappingSuggestion:{itemKeyColumn:'reference',externalReferenceColumn:'reference',amountColumn:'amount',occurredOnColumn:'date',participantColumn:'responsible',identityCandidateColumns:['reference'],ambiguousFields:[]}}
...
describe('ClosingSourceProfileConfiguration',()=>{
  ...
  it('saves the canonical source and versioned profile without receiving workbook bytes',async()=>{
    ...
    expect(serviceMock.createImportProfile).toHaveBeenCalledWith(closing,expect.objectContaining({sourceKey:'SOURCE_ONE',config:expect.objectContaining({expectedSheet:'Source one',headerRow:2,headerSignature:['reference','amount','date','responsible']})}))
