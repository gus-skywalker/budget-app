...
export default {
  ...
  confirmProfileImport(closing: FinancialClosing, file: File, batchKey: string, profileId: string, sensitiveAccessConfirmed = false, legitimateMultiplicityConfirmed = false) { const body=new FormData(); body.append('file',file);body.append('batchKey',batchKey);body.append('profileId',profileId);body.append('sensitiveAccessConfirmed',String(sensitiveAccessConfirmed));body.append('legitimateMultiplicityConfirmed',String(legitimateMultiplicityConfirmed));return axiosInterceptor.post<TabularImport>(`${versionPath(closing)}/tabular-imports/profile`,body) },
