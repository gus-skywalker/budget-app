...
export interface ClosingVersion {
  ...
  financialPublicationCurrent?: boolean
}
...
export interface GuidedImportReview { reviewId: string; reviewSourceId: string; status: string; revision: number; detailed: boolean; readyForConfirmation: boolean; alreadyConfirmed: boolean; summary: { rowsRead: number; structurallyIgnored: number; ignoredWithoutRecipientRows: number; ignoredWithoutRecipientTotal: number; importableRows: number; correctedRows: number; excludedRows: number; participantGroups: number; participantOccurrences: number; repetitionGroups: number; repetitionOccurrences: number; importableTotal: number }; monetaryFormat?: GuidedMonetaryFormatSuggestion | null; participants: GuidedParticipantGroup[]; repetitions: GuidedRepetitionGroup[]; pendingRows: GuidedPendingRow[]; dateAlternatives: Array<{ decisionKey: string; columnLabel: string; validRows: number; resolvedRows: number }>; zeroAmountGroup?: { decisionKey: string; occurrenceCount: number; excluded: boolean } | null; repetitionsAccepted: boolean; repetitionReasonCode?: string | null; blockingReasons: string[] }
export interface GuidedReviewDecision { eventKey: string; expectedRevision: number; reviewSourceId: string; decisionKey: string; action: string; participantId?: string; participantKey?: string; displayName?: string; correctedAmount?: number; correctedDate?: string; reasonCode?: string; justification?: string; saveAsProfile?: boolean }
...
export interface ImportReadinessSource { sourceId: string; sourceKey: string; displayName: string; sourceStatus?: 'REQUIRED_PENDING' | 'REQUIRED_PUBLISHED' | 'REQUIRED_NO_ACTIVITY' | 'OPTIONAL_NOT_INCLUDED' | 'OPTIONAL_IN_PROGRESS' | 'OPTIONAL_PUBLISHED'; /** deprecated client compatibility only; API exposes sourceStatus */ status?: string; expectation?: 'REQUIRED' | 'OPTIONAL'; expectedSourceRevision?: number; blockers?: string[]; allowedActions?: string[]; recommendedProfileId?: string | null; recommendedProfileName?: string | null; action?: string; detail: string }
export interface ImportReadiness { status?: 'INPUTS_PENDING' | 'INPUTS_READY'; sources: ImportReadinessSource[]; readyToCalculate: boolean; blockingSourceKeys: string[] }
...
export interface WorkbookHeaderCandidate { rowNumber: number; headers: string[] }
export interface WorkbookSheetInventory { sheetName: string; classification: 'FINANCIAL_SOURCE_PROBABLE' | 'CONSOLIDATION' | 'SUPPORT_REVIEW'; suggestedSourceKey: string; nonEmptyDataRows: number; detail: string; recommendedProfileId?: string | null; recommendedProfileName?: string | null; selectionStatus: 'AUTO_SELECTED' | 'REVIEW_REQUIRED' | 'CONSOLIDATION'; headers?: string[]; headerCandidates?: WorkbookHeaderCandidate[] | null; mappingSuggestion?: WorkbookMappingSuggestion | null; reviewReason?: 'MISSING_SOURCE' | 'MISSING_PROFILE' | 'STRUCTURE_CHANGED' | 'AMBIGUOUS_PROFILE' | 'MISSING_FINANCIAL_RULE' | 'CONSOLIDATION' | null }
...
export interface WorkbookPublication { publicationId: string; reviewId: string; status: 'MATERIALIZED' | 'FINANCIALLY_PUBLISHED' | 'CANCELLED' | 'PUBLISHED'; idempotencyKey: string; sourceCount: number; itemCount: number; additionTotal: number; reversalTotal: number; durationMs: number; replayed: boolean }
export interface MaterializedLotSource { reviewSourceId: string; sourceId: string; sourceKey: string; active: boolean; mutable: boolean; itemCount: number; additionTotal: number; reversalTotal: number; netImpact: number }
export interface MaterializedLot { publicationId: string; reviewId: string; status: 'MATERIALIZED' | 'FINANCIALLY_PUBLISHED' | 'CANCELLED' | 'PUBLISHED'; active: boolean; mutable: boolean; itemCount: number; additionTotal: number; reversalTotal: number; netImpact: number; materializedAt: string; sources: MaterializedLotSource[] }
export interface WorkbookSourceUpdatePreview { reviewId:string; reviewSourceId:string; sourceKey:string; reviewRevision:number; previewFingerprint:string; structureFingerprint:string; identityFingerprint:string; financialFingerprint:string; baselineAvailable:boolean; unchangedItems:number; changedItems:number; addedItems:number; removedItems:number; currentItemCount:number; proposedItemCount:number; currentAdditionTotal:number; currentReversalTotal:number; proposedAdditionTotal:number; proposedReversalTotal:number; reusedDecisionCount:number; invalidatedDecisionCount:number; readyToApply:boolean; blockingReasons:string[] }
export interface WorkbookSourceUpdateResult { preview:WorkbookSourceUpdatePreview; review:WorkbookReview; guidedReview:GuidedImportReview }
...
export interface FinancialPublication { inputRevision: number; calculationRunId: string; replayed: boolean }
...
export interface ClosingReserveRule { sequence:number; name:string; scope:'CLOSING'; calculationBase:'OPENING_SCOPE_BALANCE'; percentage:number; active:boolean; justification:string; nature:'RESERVE'; reserveKind:'TM'|'TI' }
...
export default {
  ...
  declareSourceNoActivity(closing: FinancialClosing, sourceId: string, expectedSourceRevision: number, justification: string) { return axiosInterceptor.post(`${versionPath(closing)}/sources/${sourceId}/no-activity`, { expectedSourceRevision, justification }) },
  reopenSource(closing: FinancialClosing, sourceId: string, expectedSourceRevision: number, justification: string) { return axiosInterceptor.post(`${versionPath(closing)}/sources/${sourceId}/reopen`, { expectedSourceRevision, justification }) },
  ...
  closingReserveRules(closing: FinancialClosing) { return axiosInterceptor.get<ClosingReserveRule[]>(`${versionPath(closing)}/deductions`) },
  upsertClosingReserveRule(closing: FinancialClosing, payload: { sequence:number; name:string; percentage:number; active:boolean; justification:string; reserveKind:'TM'|'TI' }) { return axiosInterceptor.post(`${versionPath(closing)}/deductions`, { ...payload, scope:'CLOSING', calculationBase:'OPENING_SCOPE_BALANCE', nature:'RESERVE' }) },
  ...
  addParticipantAdjustment(closing: FinancialClosing, payload: { participantId: string; sourceId: string; direction: 'INCREASE'|'DECREASE'; amount: number; justification: string; incidenceStage: 'BEFORE_DEDUCTIONS'|'AFTER_DEDUCTIONS' }) { return axiosInterceptor.post(`${versionPath(closing)}/participant-adjustments`, payload) },
  ...
  publishFinancial(closing: FinancialClosing) { return axiosInterceptor.post<FinancialPublication>(`${versionPath(closing)}/financial-publication`) },
  ...
  discardPayoutDecision(closing: FinancialClosing, id: string, expectedRevision: number) { return axiosInterceptor.post<PayoutDecision>(`/financial-closings/${closing.id}/payout-decisions/${id}/discard`, { expectedRevision }) },
  ...
  workbookInventory(closing: FinancialClosing, file: File, sensitiveAccessConfirmed = false, externalWorkbookReferencesAcknowledged = false) { const body=new FormData(); body.append('file',file); body.append('sensitiveAccessConfirmed',String(sensitiveAccessConfirmed));body.append('externalWorkbookReferencesAcknowledged',String(externalWorkbookReferencesAcknowledged)); return axiosInterceptor.post<WorkbookInventory>(`${versionPath(closing)}/workbook-inventory`,body) },
  initiateWorkbookReview(closing: FinancialClosing, file: File, sensitiveAccessConfirmed = false, externalWorkbookReferencesAcknowledged = false) { const body=new FormData();body.append('file',file);body.append('sensitiveAccessConfirmed',String(sensitiveAccessConfirmed));body.append('externalWorkbookReferencesAcknowledged',String(externalWorkbookReferencesAcknowledged));return axiosInterceptor.post<InitiatedWorkbookReview>(`${versionPath(closing)}/workbook-reviews/initiate`,body) },
  ...
  materializedLots(closing: FinancialClosing) { return axiosInterceptor.get<MaterializedLot[]>(`${versionPath(closing)}/lots`) },
  ...
  prepareWorkbookReview(closing: FinancialClosing, file: File, profileIds: string[], expectedReviewId?: string, externalWorkbookReferencesAcknowledged = false) { const body=new FormData();body.append('file',file);body.append('profileIds',JSON.stringify(profileIds));body.append('sensitiveAccessConfirmed','true');body.append('externalWorkbookReferencesAcknowledged',String(externalWorkbookReferencesAcknowledged));if(expectedReviewId)body.append('expectedReviewId',expectedReviewId);return axiosInterceptor.post<WorkbookReview>(`${versionPath(closing)}/workbook-reviews/prepare`,body) },
  previewWorkbookSourceUpdate(closingId:string,reviewId:string,reviewSourceId:string,file:File,expectedReviewRevision:number){const body=new FormData();body.append('file',file);body.append('expectedReviewRevision',String(expectedReviewRevision));body.append('sensitiveAccessConfirmed','true');return axiosInterceptor.post<WorkbookSourceUpdatePreview>(`/financial-closings/${closingId}/workbook-reviews/${reviewId}/sources/${reviewSourceId}/update-preview`,body)},
  applyWorkbookSourceUpdate(closingId:string,reviewId:string,reviewSourceId:string,file:File,expectedReviewRevision:number,previewFingerprint:string){const body=new FormData();body.append('file',file);body.append('expectedReviewRevision',String(expectedReviewRevision));body.append('previewFingerprint',previewFingerprint);body.append('sensitiveAccessConfirmed','true');return axiosInterceptor.post<WorkbookSourceUpdateResult>(`/financial-closings/${closingId}/workbook-reviews/${reviewId}/sources/${reviewSourceId}/apply-update`,body)},
  ...
  cancelWorkbookMaterialization(closingId: string, reviewId: string) { return axiosInterceptor.post<WorkbookPublication>(`/financial-closings/${closingId}/workbook-reviews/${reviewId}/cancel-materialization`) },
  cancelWorkbookSourceMaterialization(closingId: string, reviewId: string, reviewSourceId: string) { return axiosInterceptor.post(`/financial-closings/${closingId}/workbook-reviews/${reviewId}/sources/${reviewSourceId}/cancel-materialization`) },
  reopenWorkbookReview(closingId: string, reviewId: string, expectedRevision: number) { return axiosInterceptor.post<WorkbookReview>(`/financial-closings/${closingId}/workbook-reviews/${reviewId}/reopen`, { expectedRevision }) },
  ...
  startGuidedImportReview(closing: FinancialClosing, file: File, profileId: string, sensitiveAccessConfirmed = false, workbookReviewId?: string, externalWorkbookReferencesAcknowledged = false) { const body=new FormData();body.append('file',file);body.append('profileId',profileId);body.append('sensitiveAccessConfirmed',String(sensitiveAccessConfirmed));body.append('externalWorkbookReferencesAcknowledged',String(externalWorkbookReferencesAcknowledged));if(workbookReviewId)body.append('workbookReviewId',workbookReviewId);return axiosInterceptor.post<GuidedImportReview>(`${versionPath(closing)}/tabular-import-reviews`,body) },
  ...
  confirmGuidedImportReview(closing: FinancialClosing, reviewId: string, file: File, expectedReviewRevision: number, sensitiveAccessConfirmed = false) { const body=new FormData();body.append('file',file);body.append('sensitiveAccessConfirmed',String(sensitiveAccessConfirmed));body.append('expectedReviewRevision',String(expectedReviewRevision));body.append('batchKey',crypto.randomUUID());body.append('explicitFinancialConfirmation','true');return axiosInterceptor.post<TabularImport>(`${versionPath(closing)}/tabular-import-reviews/${reviewId}/confirm`,body) },
